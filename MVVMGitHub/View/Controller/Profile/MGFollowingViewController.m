...
#import "MGFollowingViewController.h"
#import "MGFollowingViewModel.h"
#import "MGUserCell.h"
#import "MGProfileViewModel.h"

@interface MGFollowingViewController ()
@property (nonatomic, strong) MGFollowingViewModel *viewModel;
@property (nonatomic, strong) UITableView *tableView;

@end

@implementation MGFollowingViewController
...
- (instancetype)initWithViewModel:(id<MGViewModelProtocol>)viewModel{
    ...
    if (self = [super init]) {
        self.viewModel = (MGFollowingViewModel *)viewModel;
    }
    return self;
}
...
- (void)bindViewModel:(id)viewModel{
    ...
    [self.tableView.binder.didSelectedCellCommand.executionSignals.switchToLatest subscribeNext:^(NSIndexPath *indexPath) {
        @strongify(self);
        OCTUser *user = self.viewModel.dataSource[indexPath.row];
